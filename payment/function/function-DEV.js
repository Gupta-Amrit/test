...
module.exports = { 
  main: async function (event, context) {
    ...
    /* Check for Headers */
    if (!headers['x-programid'] || !headers['customerid'] || !headers["content-type"] ) { 
    ...
    let config = {
      method: 'post',
      url: 'http://golden-record.dev-dataplatform.tatadigital.com/service/tdl-dp/v1/gr/fetch-customer',
      headers: { 
    ...
    .then((response1) => {
     ...
     let config = {
       method : 'put',
       url : 'http://caas.dev-r2-uc.tatadigital.com/carts/cloneCart',
       headers : {
        'Content-Type': headers["content-type"], 
     ...
     .then((response2) => {
      ...
      for(let i in bundles){
          ...
          for (let j in orderEntries) {
              ...
              let isDigital = orderEntries[j]['isDigital']
      ...
      var reserve_req = {
        reservationStock: {
          cartId: cartId,
          programId: "UniversalCart", //programId,
      ...
      let config = {
        method : 'POST',
        url : 'http://inventory.dev-r2-uc.tatadigital.com/reserve',
        headers: {
          'Content-Type': headers["content-type"], 
          'X-ProgramID': headers["x-programid"] 
        },
      ...
      .then((response3) => {
        ...
        var merchant = {
          "merchantOrderRef": cartId, //programId, params['cartId'],
        ...
        let config = {
          method : 'post',
          url: 'http://order.dev-r2-payment.tatadigital.com/merchant/createOrderForTcp',
          headers: { 
            'Content-Type': headers["content-type"],
          },
          data : frontend_data
