...
module.exports = { 
  main: async function (event, context) {
    ...
    var inventory_config = {
      method : 'post',
      url : 'http://inventory.sit-2-uc.tatadigital.com/inventory/status',
      headers : {
    ...
    .then(function (response) {
      ...
      if (inventory_response.code === "404") {
        ...
        for (let v=0; v<validate_datails.length; v++) {
          ...
          var validate_config = {
            method : 'post',
            url : 'http://promotions.sit-offer.tatadigital.com/rest/offers/validateOffers',
            headers : {
              "Content-Type" : "application/json"
            },
            data : validate_data
          ...
          .then(function (response) {
            ...
            var edd_config = {
              method : 'post',
              url : 'http://caas-serviceability.sit-2-uc.tatadigital.com/edd',
              headers : {
                "X-ProgramID" : headers['x-programid'],
                "Content-Type" : "application/json"
              },
            ...
            .then(function (response) {
              ...
              if (overrideBundle.toLowerCase() === "yes") {
                ...
                var updateCart_config = {
                  method : 'post',
                  url : 'http://caas.sit-2-uc.tatadigital.com/carts/updateCart',
                  headers : {
                    "X-ProgramID" : headers['x-programid'],
                ...
                var addToCart_config = {
                  method : 'post',
                  url : 'http://caas.sit-2-uc.tatadigital.com/carts/addToCart',
                  headers : {
                    "X-ProgramID" : headers['x-programid'],
                    "customerId" : headers['customerid'],
                    "Content-Type" : "application/json"
                ...
                .catch(function (error) {
                  if (error) {
                    let addToCart_error = {
                      ...
                      message : "ERROR",
                      error : {
                        message : "Sorry! Bundles is already available in cart"
                      },
                      code : "ERR005"
                    }
